  ...
  baseOffsets: Array.from({ length: amount }, (_, i) => {
    ...
    return {
      ...
      posSpeed: new THREE.Vector3(
        0.1 + Math.random() * 0.3, // Much slower movement
        0.1 + Math.random() * 0.2,
        0.05 + Math.random() * 0.1
      ),
      rotSpeed: new THREE.Vector3(
        0.01 + Math.random() * 0.03, // Much slower rotation
...
void main() {
  ...
  if(totalDist < tMax) {
    ...
    // Primary color for blobs (slate-700: #334155)
    vec3 baseColor = vec3(0.2, 0.255, 0.333); // slate-700 RGB values

    // Subtle lighting for depth
    color = baseColor * (0.6 + diff * 0.3);
    color += vec3(0.4, 0.45, 0.5) * spec * 0.2; // Subtle specular

    // Very subtle edge glow to define the shape
    float rimLight = pow(1.0 - max(dot(normal, viewDir), 0.0), 2.0);
    color += vec3(0.1, 0.12, 0.15) * rimLight;

    // Make geometry visible but subtle
    alpha = 0.7; // Semi-transparent for subtlety
  }
  ...
  useFrame((_, delta) => {
    if (materialRef.current) {
      ...
      animationState.baseOffsets.forEach((offset, i) => {
        const wanderX = Math.sin(time * offset.posSpeed.x + offset.posPhase.x) * 0.3
        const wanderY = Math.cos(time * offset.posSpeed.y + offset.posPhase.y) * 2.0
        const wanderZ = Math.sin(time * offset.posSpeed.z + offset.posPhase.z) * 0.2

        const secondaryX = Math.cos(time * offset.posSpeed.x * 0.5 + offset.posPhase.x * 1.1) * 0.15
        const secondaryY = Math.sin(time * offset.posSpeed.y * 0.6 + offset.posPhase.y * 0.9) * 0.12
